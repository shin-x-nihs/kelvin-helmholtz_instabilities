program KHI_OpenMP_Benchmark
    ...
    print *, "Initializing 512x512 CPU Benchmark (OpenMP)..."
    call init_timer()
    ...
    do j = 1, ny
        do i = 1, nx
            y = (real(j) - 0.5) * dy        ! Center of cell in y-direction: "0.5" to be in the center of the cell, not the edge
            x = (real(i) - 0.5) * dx        ! Center of cell in x-direction: "0.5" to be in the center of the cell, not the edge
            ...
            v = 0.05 * sin(4.0 * 3.14159265 * x) * &                            ! Little "noise" to seed the instability
                (exp(-(y-0.25)**2 / 0.005) + exp(-(y-0.75)**2 / 0.005))        ! Localized around the shear layers at y=0.25 and y=0.75
    ...
    call print_timing_results(nx, ny, "OpenMP")
...
contains
    ...
    subroutine compute_dt(state, nx, ny, ng, dx, dy, gamma, cfl, dt_out)
        ...
        !$omp parallel do collapse(2) reduction(max:max_speed) private(i, j, rho, u, v, P, c)
        do j = 1, ny
            do i = 1, nx
                rho = state(1, i, j)
                u = state(2, i, j) / rho
    ...
    subroutine compute_rhs(state, rhs, F_ext, G_ext, nx, ny, ng, dx, dy, gamma)
        ...
        !$omp parallel do collapse(2) private(i, j, k, d, prim, qL, qR, &
        !$omp& rhoL, uL, vL, PL, EL, cL, rhoR, uR, vR, PR, ER, cR, &
        !$omp& SL, SR, SM, PM, U_star_L, U_star_R, fL_flux, fR_flux)
        do j = 1, ny
            do i = 0, nx
                do k = -2, 3
                ...
                if (SL >= 0.0) then
                    ...
                    if (SM >= 0.0) then
                        ...
                        U_star_L = [rhoL*(SL - uL)/(SL - SM), &
                                    rhoL*(SL - uL)/(SL - SM)*SM, &
                                    rhoL*(SL - uL)/(SL - SM)*vL, &
                                    rhoL*(SL - uL)/(SL - SM)*(EL/rhoL + (SM - uL)*(SM + PL/(rhoL*(SL - uL))))]
                        F_ext(:, i, j) = fL_flux + SL * (U_star_L - [rhoL, rhoL*uL, rhoL*vL, EL])
                    else
                        PM = rhoR*(uR - SR)*(uR - SM) + PR
                        U_star_R = [rhoR*(SR - uR)/(SR - SM), &
                                    rhoR*(SR - uR)/(SR - SM)*SM, &
                                    rhoR*(SR - uR)/(SR - SM)*vR, &
                                    rhoR*(SR - uR)/(SR - SM)*(ER/rhoR + (SM - uR)*(SM + PR/(rhoR*(SR - uR))))]
            ...
            do j = 0, ny
                ...
                if (SL >= 0.0) then
                    ...
                    if (SM >= 0.0) then
                        ...
                        U_star_L = [rhoL*(SL - vL)/(SL - SM), &
                                    rhoL*(SL - vL)/(SL - SM)*uL, &
                                    rhoL*(SL - vL)/(SL - SM)*SM, &
                                    rhoL*(SL - vL)/(SL - SM)*(EL/rhoL + (SM - vL)*(SM + PL/(rhoL*(SL - vL))))]
                        G_ext(:, i, j) = fL_flux + SL * (U_star_L - [rhoL, rhoL*uL, rhoL*vL, EL])
                    else
                        PM = rhoR*(vR - SR)*(vR - SM) + PR
                        U_star_R = [rhoR*(SR - vR)/(SR - SM), &
                                    rhoR*(SR - vR)/(SR - SM)*uR, &
                                    rhoR*(SR - vR)/(SR - SM)*SM, &
                                    rhoR*(SR - vR)/(SR - SM)*(ER/rhoR + (SM - vR)*(SM + PR/(rhoR*(SR - vR))))]
...
end program KHI_OpenMP_Benchmark